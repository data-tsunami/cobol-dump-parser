 ...
 * 
 * Takes a string argument specifying the ranges of each column in a unix
 * 'cut'-like format. Ex: '-5, 10-12, 14, 20-' Ranges are comma-separated,
 * 1-indexed (for ease of use with 1-indexed text editors), and inclusive. A
 * single-column field at position n may be specified as either 'n-n' or simply
 * 'n'.
 * 
 * A second optional argument specifies whether to skip the first row of the
 * input file, assuming it to be a header. As Pig may combine multiple input
 * files each with their own header into a single split, FixedWidthLoader makes
 * sure to skip any duplicate headers as will. 'SKIP_HEADER' skips the row;
 * anything else and the default behavior ('USE_HEADER') is not to skip it.
 * 
 * A third optional argument specifies a Pig schema to load the data with.
 * Automatically trims whitespace from numeric fields. Note that if fewer fields
 * are specified in the schema than are specified in the column spec, only the
 * fields in the schema will be used.
 * 
 * Warning: fields loaded as char/byte arrays will trim all leading and trailing
 * whitespace from the field value as it is indistiguishable from the spaces
 * that separate different fields.
 * 
 * All datetimes are converted to UTC when loaded.
 * 
 * Column spec idea and syntax parser borrowed from Russ Lankenau's
 * implementation at https://github.com/rlankenau/fixed-width-pig-loader
 */
public class FixedWidthLoader extends LoadFunc implements LoadMetadata, LoadPushDown {
	...
	public FixedWidthLoader() {
		throw new IllegalArgumentException("Usage: org.apache.pig.piggybank.storage.FixedWidthLoader("
		...
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (fieldType == DataType.MAP || fieldType == DataType.TUPLE || fieldType == DataType.BAG) {
					throw new IllegalArgumentException("Field \"" + fields[i].getName()
							+ "\" is an object type (map, tuple, or bag). "
			...
			if (fields.length < columns.size())
				...
				throw new IllegalArgumentException(
						"More fields specified in schema than columns specified in column spec.");
		} catch (ParserException e) {
			throw new IllegalArgumentException("Invalid schema format: " + e.getMessage());
		}
	...
	public static ArrayList<FixedWidthField> parseColumnSpec(String spec) {
		...
		for (String range : ranges) {
			...
			// but we convert to 0-indexing and inclusive-exclusive [)
			// internally
			...
			if (range.indexOf("-") != -1) {
				...
				if (start + 1 < 1)
					throw new IllegalArgumentException("Illegal column spec '" + range
							+ "': start value must be at least 1");
				if (start + 1 > end)
					throw new IllegalArgumentException("Illegal column spec '" + range
	...
	public Tuple getNext() throws IOException {
		...
		try {
			...
			// if the line is a duplicate header and 'SKIP_HEADER' is set,
			// ignore it
			// (this might happen if multiple files each with a header are
			// combined into a single split)
			...
			for (int i = 0; i < fields.length; i++) {
				if (requiredFields[i]) {
					try {
						...
						warn("Exception when parsing field \"" + fields[i].getName() + "\" " + "in record "
								+ line.toString() + ": " + e.toString(), PigWarning.UDF_WARNING_1);
					}
					count++;
				}
			}
	...
	private Object readField(String line, ResourceFieldSchema field, FixedWidthField column)
		...
		case DataType.DATETIME:
			throw new IllegalArgumentException(
					"Object types (DATETIME) are not supported by FixedWidthLoader");

		case DataType.MAP:
		case DataType.TUPLE:
		case DataType.BAG:
			throw new IllegalArgumentException(
					"Object types (map, tuple, bag) are not supported by FixedWidthLoader");
